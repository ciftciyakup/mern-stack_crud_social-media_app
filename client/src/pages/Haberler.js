...
const Haberler = () => {
  ...
  const { newsList, status, updateNewsList } = useSelector(
    (state) => state.news
  );
  ...
  useEffect(() => {
    window.scrollTo({ top: 0, left: 0, behavior: "smooth" }); // Sayfa yüklendiğinde en üse çık
    dispatch(
      getNews({
                ...
                >
                  ...
                  {user?.userType === "admin" && (
                    <div className="mt-auto flex justify-between items-center">
                      ...
                      >
                        <Edit />{" "}
                        {/* FontAwesome ikonları yerine Material UI ikonları */}
                      </Button>
                      <Button
                        variant="contained"
                        color="error"
