...
import {
  addNewChat,
  resetNewChat,
  clearErrors as clearNewChatErrors,
} from "../../slices/newChatSlice";
import { toast } from "react-hot-toast";
import { clearErrors as clearAllChatsErrors } from "../../slices/allChatsSlice";
...
const NewDialog = ({ open, onClose }) => {
  ...
  const fetchUsers = async (term) => {
    setLoading(true);
    const { data } = await axios.get(`/users?keyword=${term}`);
    setUsers(
      data.users.filter(
        (u) => u._id !== self._id && u.userType !== "guest" && u.userType !== "turnuvaKayit"
      )
    );
    setLoading(false);
  };
  ...
  useEffect(() => {
    if (error) {
      toast.error(error);
      dispatch(clearAllChatsErrors());
      dispatch(clearNewChatErrors());
    }
    if (chat) {
      const friendId = chat.users?.find((id) => id !== self._id);
      navigate(`/sosyal/direct/t/${chat._id}/${friendId}`);
    ...
    <Dialog open={open} onClose={onClose}>
      <div className="flex flex-col w-80 sm:w-96">
        <div className="flex justify-between items-center px-4 border-b py-2.5">
          <span className="font-medium mx-auto">Yeni Mesaj</span>
          <svg
            onClick={onClose}
              ...
              >
                <div className="flex space-x-3 items-center">
                  ...
                  <div className="flex flex-col items-start">
                    <span className="text-black text-sm font-semibold">{u.fullname}</span>
                  </div>
                </div>
              </div>
            ))
          ) : (
            <span className="text-gray-400 text-sm p-2">Hiçbir hesap bulunamadı.</span>
          )}
        </div>
      </div>
    </Dialog>
